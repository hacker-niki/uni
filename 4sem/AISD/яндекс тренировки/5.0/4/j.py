...
def check(m):
    for i in range(1, n+1):
        if y[i-1] > y[i] < y[i+1]:
            square = get_prevleft(i, y[i] + m) + get_prevright(i, y[i] + m)
            if square >= 0:
                return True
    return False
...
for i in range(n,0,-1):
    sufadd[i] = max(0, get_prevright(i, y[i]))
prefadd = [0] * (n+2)
for i in range(1, n+1):
    prefadd[i] = max(0, get_prevleft(i, y[i]))
