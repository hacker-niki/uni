...
def analyze_continuous_data(samples):
    ...
    # --- НАЧАЛО ИЗМЕНЕНИЙ: Расчет теоретической маргинальной плотности ---
    ...
    x_theory = np.linspace(-2.99, 2.99, 150) # отступаем от краев для стабильности интегратора
    pdf_x_theory = []
    ...
    ax1.plot(x_theory, pdf_x_theory, 'r-', lw=2, label='Теоретическая') # <-- ДОБАВЛЕНО
    ax1.set_title("Маргинальное распределение X")
    ax1.grid(alpha=0.3)
    ax1.legend() # <-- ДОБАВЛЕНО

    # Гистограмма Y
    ax2 = fig.add_subplot(2, 2, 2)
    ax2.hist(y, bins=30, density=True, alpha=0.7, color='lightgreen', label='Эмпирическая')
    ax2.plot(x_theory, pdf_x_theory, 'r-', lw=2, label='Теоретическая') # <-- ДОБАВЛЕНО
    ax2.set_title("Маргинальное распределение Y")
    ax2.grid(alpha=0.3)
    ax2.legend() # <-- ДОБАВЛЕНО
    
    # 3D гистограмма (эмпирическая)
    ...
    ax3.set_title("Эмпирическая 2D плотность")
    ax3.set_zlim(0, None)
    
    # 3D теоретическая плотность
    ...
    Z = (1 / (63 * np.pi)) * (9 - R)
    Z[R > 3] = np.nan # Убираем значения вне круга
    ax4.plot_surface(X_g, Y_g, Z, cmap=cm.plasma, alpha=0.9)
    ax4.set_title("Теоретическая 2D плотность")
    ax4.set_zlim(0, None)
    ...
    output = ["=== Теоретические характеристики ===", f"  E[X] = {E_X:.4f}, E[Y] = {E_Y:.4f}", f"  Var[X] = {Var_X:.4f}, Var[Y] = {Var_Y:.4f}",
              ...
              f"  Mean X = {mean_x:.4f}, Mean Y = {mean_y:.4f}", f"  Var X = {var_x:.4f}, Var Y = {var_y:.4f}",
              f"  Cov(X, Y) = {cov_xy:.4f}", f"  Corr(X, Y) = {corr_xy:.4f}\n", "=== Проверка гипотез (p-value) ===",
...
def analyze_discrete_data(samples, P):
    ...
    # Формируем отчет (здесь можно добавить больше деталей)
    output = [
